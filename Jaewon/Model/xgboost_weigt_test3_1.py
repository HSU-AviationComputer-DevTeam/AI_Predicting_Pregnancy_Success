...
X['배아_난자_상호작용'] = X['이식된 배아 수'] * X['난자 출처']
test['배아_난자_상호작용'] = test['이식된 배아 수'] * test['난자 출처']
...
xgb = XGBClassifier(use_label_encoder=False, eval_metric='auc', random_state=42, scale_pos_weight=np.sqrt(sum(y == 0) / sum(y == 1)))
...
# 테스트 데이터 예측
pred_proba = best_model.predict_proba(test_imputed)[:, 1]

# 제출 파일 생성
sample_submission = pd.read_csv('C:/code/aimers/data/sample_submission.csv')
sample_submission['probability'] = pred_proba
sample_submission.to_csv('C:/code/aimers/data/xgboost_weight_test3-1.csv', index=False)
...
"""
결과
최적 하이퍼파라미터: {'colsample_bytree': 0.7, 'learning_rate': 0.025, 'max_depth': 5, 'n_estimators': 500, 'subsample': 0.7}
최고 AUC 점수: 0.7396320599362562
"""