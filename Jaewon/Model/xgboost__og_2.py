...
plt.rcParams['font.family'] = 'Malgun Gothic'  # plt 깨짐방지
...
if 'ID' in train.columns:
    train.drop(columns=['ID', '난자 해동 경과일', '난자 채취 경과일', '불임 원인 - 자궁경부 문제', '불임 원인 - 정자 면역학적 요인', '불임 원인 - 여성 요인'], inplace=True)
if 'ID' in test.columns:
    test.drop(columns=['ID', '난자 해동 경과일', '난자 채취 경과일', '불임 원인 - 자궁경부 문제', '불임 원인 - 정자 면역학적 요인', '불임 원인 - 여성 요인'], inplace=True)
...
#  XGBoost 하이퍼파라미터 튜닝 (GridSearchCV 사용)
...
sample_submission.to_csv('C:/code/aimers/data/xgboost_og_2_submit.csv', index=False)
...
####
#수정내역 - 불필요 컬럼 드랍 
# 'ID', '난자 해동 경과일', '난자 채취 경과일', '불임 원인 - 자궁경부 문제', '불임 원인 - 정자 면역학적 요인', '불임 원인 - 여성 요인'  
# 코드결과
#최적 하이퍼파라미터: {'colsample_bytree': 0.8, 'learning_rate': 0.025, 'max_depth': 5, 'n_estimators': 500, 'subsample': 0.7}
# 최고 AUC 점수: 0.7395602282596806
