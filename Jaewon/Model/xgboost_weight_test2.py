...
# 가중치를 부여할 중요한 컬럼
important_features = ['이식된 배아 수', '배아 이식 경과일', '총 생성 배아 수', '혼합된 난자 수', '단일 배아 이식 여부', '파트너 정자와 혼합된 난자 수', '미세주입에서 생성된 배아 수']
...
sample_submission.to_csv('C:/code/aimers/data/xgboost_weight_test2.csv', index=False)
...
최고 AUC 점수: 0.7396183254017823
최적 하이퍼파라미터: {'colsample_bytree': 0.8, 'learning_rate': 0.05, 'max_depth': 4, 'n_estimators': 400, 'subsample': 0.7}



# 수정
xgboost_weight_test 에서의 이식배아경과일 상호작용 피쳐 삭제   +    가중치를 부여할 중요한 컬럼 변경 7개 
['이식된 배아 수', '배아 이식 경과일', '총 생성 배아 수', '혼합된 난자 수', '단일 배아 이식 여부', '파트너 정자와 혼합된 난자 수', '미세주입에서 생성된 배아 수']



# 결과
최적 하이퍼파라미터: {'colsample_bytree': 0.6, 'learning_rate': 0.05, 'max_depth': 4, 'n_estimators': 400, 'subsample': 0.7}
최고 AUC 점수: 0.7394863070596859
